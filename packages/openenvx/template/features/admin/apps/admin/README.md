...
Browser → Middleware (cookie check) → Login Page (if not authenticated)
   ↓
Admin API → withAuth (JWT validation) → 401 Error (if invalid)
   ↓ (valid)
...
This app imports auth from `@{{projectName}}/auth`. Make sure it exports an `AuthClient`:
...
```typescript
import { withAuth, createAdmin } from "@openenvx/admin/server";
